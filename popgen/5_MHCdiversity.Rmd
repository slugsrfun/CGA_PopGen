...
This chunk of code creates `.bed` files for extracting the markers around exons 2 and 6 with `vcftools`. We'll start with the `PopGen0.recode.vcf` file that served as the beginning point for our filtering in `18_filterPopGen`. This file has had duplicate SNPs, half-calls, indels, and duplicate genotypes (as identified in `10_recapFinal`) removed, but no other filters applied. First, we extract SNPs within these regions to identify boundaries of the translated sequence, later we will apply some filters for SNP quality and genotype depth.  
...
nrow(biallelic)
write.table(biallelic, file = "DQBex2_biallelic.txt",
...
# SNP-based summaries
Returning to the unphased `.vcf` file (without genotype imputation) for SNP-based genetic diversity measures.  
```{r engine="bash", comment=""}
plink --vcf DQBex2_final.recode.vcf --recodeA --out DQBex2 \
      --double-id --allow-extra-chr --silent
```

Now read the `.raw` file in and create objects for each population (accounting for survivors)...  
```{r createSNPobj}
rawsnp <- read.table("DQBex2.raw", header = TRUE)
which(colnames(rawsnp) == "PHENOTYPE")
DQBsnp <- rawsnp[,-c(1:6)]
DQBsnp[is.na(DQBsnp)] <- -1

colnames(DQBsnp) <- paste0("loc",c(1:ncol(DQBsnp)))

nrow(rawsnp)
sum(hap.out$vcfID == rawsnp$IID)

DQBmeta <- data.frame(pop=hap.out$pop,
                      vcfID=hap.out$vcfID,
                      survivor=hap.out$survivor)

#Use the hap.out object since the rows are in the same order
DQB <- cbind(DQBmeta, DQBsnp)
DQB_NGA <- DQB[which(DQB$pop == "NGA"),]
nrow(DQB_NGA)
DQB_SGA <- DQB[which(DQB$pop == "SGA"),]
nrow(DQB_SGA)
DQB_CGA12 <- DQB[which(DQB$pop == "CGA-H" | DQB$survivor == TRUE),]
nrow(DQB_CGA12)
DQB_CGA23 <- DQB[which(DQB$pop == "CGA" | DQB$survivor == TRUE),]
nrow(DQB_CGA23)
```
...
sum(apply(DQB_NGA[,-c(1:3)], 2, FUN=is.poly))
sum(apply(DQB_CGA12[,-c(1:3)], 2, FUN=is.poly))
sum(apply(DQB_CGA23[,-c(1:3)], 2, FUN=is.poly))
sum(apply(DQB_SGA[,-c(1:3)], 2, FUN=is.poly))
```

```{r compPoly, echo = FALSE}
temp <- data.frame(NGA = sum(apply(DQB_NGA[,-c(1:3)], 2, FUN=is.poly)),
                  CGA12 = sum(apply(DQB_CGA12[,-c(1:3)], 2, FUN=is.poly)),
                  CGA23 = sum(apply(DQB_CGA23[,-c(1:3)], 2, FUN=is.poly)),
                  SGA = sum(apply(DQB_SGA[,-c(1:3)], 2, FUN=is.poly)))
out <- rbind(out, temp)
rownames(out) <- c("n","K_hap","H_hap","Ho_hap","K_snp")
rm(temp)
```

## Expected heterozygosity ($H_e$)
Calculate allele frequencies per locus, then expected heterozygosity of that locus. We include comparisons across populations at all SNPs in the `.vcf` file and comparisons that limit the analysis to SNP loci that were polymorhpic in all three populations (central, north, and south Georgia)...  
```{r He}
getHe <- function(x) {
...
mean(apply(DQB_NGA[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE)
mean(apply(DQB_CGA12[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE)
mean(apply(DQB_CGA23[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE)
mean(apply(DQB_SGA[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE)

#What if we only consider SNPs that are polymorphic in all populations?
polymorphic <- data.frame(NGA = apply(DQB_NGA[,-c(1:3)], 2, FUN=is.poly),
                          CGA = apply(DQB_CGA23[,-c(1:3)], 2, FUN=is.poly),
                          SGA = apply(DQB_SGA[,-c(1:3)], 2, FUN=is.poly))
allpolySNPs <- which(rowSums(polymorphic) == 3)

#More comparable if we limit to SNPs that are polymorhpic in all pops?
mean(apply(DQB_NGA[,allpolySNPs], 2, FUN=getHe), na.rm = TRUE)
mean(apply(DQB_CGA12[,allpolySNPs], 2, FUN=getHe), na.rm = TRUE)
mean(apply(DQB_CGA23[,allpolySNPs], 2, FUN=getHe), na.rm = TRUE)
mean(apply(DQB_SGA[,allpolySNPs], 2, FUN=getHe), na.rm = TRUE)

```

Limiting to SNPs that are polymorphic in all populations has some minor effects on $H_e$, as expected, increasing the values in the undersampled populations slightly.  
```{r compHe, echo = FALSE}
temp <- data.frame(NGA=mean(apply(DQB_NGA[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE),
                   CGA12=mean(apply(DQB_CGA12[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE),
                   CGA23=mean(apply(DQB_CGA23[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE),
                   SGA=mean(apply(DQB_SGA[,-c(1:3)], 2, FUN=getHe), na.rm = TRUE))
out <- rbind(out, temp)
rownames(out) <- c("n","K_hap","H_hap","Ho_hap","K_snp", "He_snp")
rm(temp)
```

## Observed Heterozygosity ($H_o$)
Calculate observed heterozygosity (# hets / # genotyped) per locus, then average across loci. Again we include estimates using all SNPs and those using only the `r length(allpolySNPs)` loci that were polymorhpic in all three populations.  
```{r Ho}
...
mean(apply(DQB_NGA[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE)
mean(apply(DQB_CGA12[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE)
mean(apply(DQB_CGA23[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE)
mean(apply(DQB_SGA[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE)

#More comparable if we limit to SNPs that are polymorhpic in all pops?
mean(apply(DQB_NGA[,allpolySNPs], 2, FUN=getHo), na.rm = TRUE)
mean(apply(DQB_CGA12[,allpolySNPs], 2, FUN=getHo), na.rm = TRUE)
mean(apply(DQB_CGA23[,allpolySNPs], 2, FUN=getHo), na.rm = TRUE)
mean(apply(DQB_SGA[,allpolySNPs], 2, FUN=getHo), na.rm = TRUE)
```
Limiting the dataset to SNPs that are polymorphic across the three GA populations does not have major impacts on the $H_o$ values. With that in mind, we'll use the full set of SNPs in the summary report at the end of the markdown.  

```{r compHo, echo = FALSE}
temp <- c(mean(apply(DQB_NGA[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE),
          mean(apply(DQB_CGA12[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE),
          mean(apply(DQB_CGA23[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE),
          mean(apply(DQB_SGA[,-c(1:3)], 2, FUN=getHo), na.rm = TRUE))
out <- rbind(out, temp)
rownames(out) <- c("n","K_hap","H_hap","Ho_hap","K_snp", "He_snp", "Ho_snp")
rm(temp)
```

## Allelic Richness  
With rarefaction to a sample smaller than the smallest sample size in the dataset...  

First sort the VCF by population...  
```{r makeMHCkeep}
...
sed -e 's/UGA/UGAh/g' DQBex2_CGA12.recode.vcf > DQBex2_CGA12h.recode.vcf

#Grab header and SNP rows
grep -v ^## DQBex2_CGA12h.recode.vcf > DQB_CGA12snps
grep -v ^## DQBex2_CGA23.recode.vcf | cut -f 10- > DQB_CGA23snps
grep -v ^## DQBex2_NGA.recode.vcf | cut -f 10- > DQB_NGAsnps
grep -v ^## DQBex2_SGA.recode.vcf | cut -f 10- > DQB_SGAsnps

#Paste data columns together
paste DQB_CGA12snps DQB_CGA23snps DQB_NGAsnps DQB_SGAsnps > DQBex2_tempcombined

#Grab header from one of the files
grep ^## DQBex2_CGA12.recode.vcf > DQBex2_combined.vcf
cat DQBex2_tempcombined >> DQBex2_combined.vcf

#Make an individual ID file
grep -v ^## DQBex2_combined.vcf | head -1 | tr '\t' '\n' | grep ^UGA > mhcIID.txt
```
...
IID <- read.table("mhcIID.txt", header = FALSE)
FID <- c()
for(i in 1:nrow(IID)) {
...
vcf <- read.vcfR("DQBex2_combined.vcf", verbose=TRUE)
my_genind <- vcfR2genind(vcf, pop=plinkPops$pop)

n.matrix <- data.frame(
  CGA12 = colSums(!is.na(my_genind@tab[which(my_genind@pop == "CGA-H"),])),
  CGA23 = colSums(!is.na(my_genind@tab[which(my_genind@pop == "CGA"),])),
...
```{r compAR, echo = FALSE}
temp <- rowMeans(AR)[c("NGA","CGA-H","CGA","SGA")]
out <- rbind(out, temp)
rownames(out) <- c("n","K_hap","H_hap","Ho_hap","K_snp", "He_snp", "Ho_snp","AR")
rm(temp)
```
...
out <- rbind(out, temp)
rownames(out) <- c("n","K_hap","H_hap","Ho_hap","K_snp", "He_snp", "Ho_snp","AR","missense")
rm(temp)
```
...
# Summary
Diversity at SNPs in exon 2 of the MHC class II gene *DQB* is lower in central GA than in north GA (particularly when considering $H_e$, $H_o$, and allelic richness). However, haplotype diversity and observed heterozygosity at the haplotype-level is still fairly high in central GA. Overall, central GA has retained a surprising amount of diversity in exon 2 of *DQB*. Furthermore, the haplotype network and neighbor-joining tree show that variants segregating in the central GA population are not particularly tightly clustered.  
```{r kable}
knitr::kable(t(out), digits = c(0,0,3,3,0,3,3,2,0))
```
